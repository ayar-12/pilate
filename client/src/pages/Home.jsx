...
function Home() {
    ...
    // Memoize latestCourses to prevent unnecessary re-renders
    ...
    // Memoize latestBlog to prevent unnecessary re-renders
    const latestBlog = useMemo(() => {
        return blogs && blogs.length > 0 ? blogs[0] : null;
    }, [blogs]);

    // Flash effect for "New Class" label
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (homeData?.video) {
            console.log("Video URL:", homeData.video);
        }
    }, [homeData, backendUrl]);
    ...
    // Course carousel effect - only run when latestCourses changes
    useEffect(() => {
        ...
        return () => clearInterval(interval);
    }, [latestCourses.length]); // Only depend on length, not the entire array

    // Memoize video URL to prevent recalculation
    const videoUrl = useMemo(() => {
        if (!homeData?.video) return '';
        
        if (homeData.video.startsWith('http')) {
            return homeData.video;
        }
    ...
    useEffect(() => {
        if (videoUrl) {
            ...
            console.warn("⚠️ No video URL — check homeData or backendUrl");
        }
    }, [videoUrl]);
    ...
    if (!courses || latestCourses.length === 0) return null;
                                            ...
                                            <p
                                                style={{
                                                    ...
                                                    backgroundColor: flashRed
                                                        ? 'rgba(195, 94, 151, 0.56)'
                                                        : '#ffffff',
                                                    color: flashRed ? '#f0edefff' : '#8d1f58',
